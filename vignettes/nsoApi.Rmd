...
```{r provider-overview, eval=TRUE, echo=FALSE, results="asis"}
## getwd()
...
```{r beaapi-getparametervalues, eval=TRUE, echo=TRUE}
api.param <- list(
...
```{r beaapi-paramvalue, eval=TRUE, echo=FALSE, results="asis"}
knitr::kable(tablelist$BEAAPI$Results$ParamValue[1:n_table, ])
```

```{r beaapi-getdata, echo=TRUE, eval=TRUE, results="asis"}
api.param <- list(
  DATASETNAME = "GDPBYINDUSTRY",
  FREQUENCY = "A",
  INDUSTRY = "11",
...
```{r beaapi-data, eval=TRUE, results="asis"}
knitr::kable(datalist$BEAAPI$Results$Data)
```
...
```{r cbs-dimension, eval=TRUE, echo=TRUE, results="asis"}
...
```{r cbs-data, eval=TRUE, echo=TRUE, results="asis"}
scheme <- "TypedDataSet"
# print query
cbsODataAPI(api=api, DSD=DSD, scheme=scheme, query=TRUE)
...
```{r cbs-tables, eval=TRUE, echo=TRUE, results="asis"}
...
```{r genesis-metadata, eval=TRUE, echo=TRUE, results="asis"}
...
```{r genesis-data, eval=TRUE, echo=TRUE, results="asis"}
api.param.datenexport <- list(
    method = "DatenExport",
    kennung = apiKey$GENESIS$kennung,
    passwort = apiKey$GENESIS$passwort,
...
```{r genesis-tables, eval=TRUE, echo=TRUE, results="asis"}
...
```{r ons-data, eval=TRUE, echo=TRUE, results="asis"}
...
```{r ons-tables, eval=TRUE, echo=TRUE, results="asis"}
